...
void Qwen3MoeModel::load_weights_to_graph(CactusGraph* gb) {
    ...
    for (uint32_t i = 0; i < config_.num_layers; i++) {
        ...
        for (uint32_t e = 0; e < config_.num_experts; e++) {
            auto& expert = layer.moe_experts[e];
            std::string expert_prefix = layer_prefix + "moe_expert_" + std::to_string(e) + "_";
            expert.w1_weight = gb->mmap_weights(expert_prefix + "w1.weights");
            expert.w3_weight = gb->mmap_weights(expert_prefix + "w3.weights");
            expert.w2_weight = gb->mmap_weights(expert_prefix + "w2.weights");
        }
    }
}
...
size_t Qwen3MoeModel::build_attention(CactusGraph* gb, size_t normalized_input, uint32_t layer_idx,
    ...
    // QK normalization per-head before RoPE
    ...
    auto attn_output = gb->reshape(attn_output_4d, {seq_len, head_dim * num_heads});
...
size_t Qwen3MoeModel::build_mlp(CactusGraph* gb, size_t normalized_h, uint32_t layer_idx,
    ...
    auto topk_result = gb->topk(routing_probs, config_.num_experts_per_tok);
    ...
    for (uint32_t e = 0; e < config_.num_experts; ++e) {
        const auto& expert = layer.moe_experts[e];
